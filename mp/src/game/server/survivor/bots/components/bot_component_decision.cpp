...
void CBotDecision::SwitchToBestWeapon()
{
    ...
    for ( int i = 0; i < GetHost()->WeaponCount(); i++ ) {
        ...
        if (pWeapon->IsPistol() || pWeapon->IsDualPistol()) {
        ...
        // The best sniper
        else if ( pWeapon->IsSniper() || pWeapon->IsCrossbow()) {
        ...
        // The best shotgun
        else if ( pWeapon->IsShotgun() || pWeapon->IsShotgunDouble()) {
    ...
    if ( memory ) {
        ...
        if ( (pCurrent->IsSniper() || pCurrent->IsCrossbow()) && pShortRange ) {
        ...
        if ((!pCurrent->IsSniper() || !pCurrent->IsCrossbow()) && pSniper ) {
...
bool CBotDecision::GetNearestCover( float radius, Vector *vecCoverSpot ) const
{
    ...
    if ( GetHost()->GetActiveBaseWeapon() && (GetHost()->GetActiveBaseWeapon()->IsSniper() || GetHost()->GetActiveBaseWeapon()->IsCrossbow())) {
...
BCOND CBotDecision::ShouldRangeAttack2()
{
    ...
    // Snipa!
    if ( pWeapon->IsSniper() || pWeapon->IsCrossbow()) {
        if ( !GetProfile()->IsEasiest() ) {
            // Zoom!
