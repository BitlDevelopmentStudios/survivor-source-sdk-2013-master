...
DECLARE_DEBUG_COMMAND( bot_secondary_attack, "0", "" )
...
DECLARE_DEBUG_COMMAND( bot_class, "0", "Force all bots created with bot_add to change to the specified class")
DECLARE_DEBUG_COMMAND( bot_faction, "0", "Force all bots created with bot_add to change to the specified faction")
DECLARE_DEBUG_COMMAND( bot_gender, "0", "Force all bots created with bot_add to change to the specified gender")
...
{
    if ( !pPlayername ) {
        const char *pPlayername1 = m_botNames[RandomInt( 0, ARRAYSIZE( m_botNames ) - 1 )];
        pPlayername = UTIL_VarArgs("%s%s", pPlayername1);
    }
