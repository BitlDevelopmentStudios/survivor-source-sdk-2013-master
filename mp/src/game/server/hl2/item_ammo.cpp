...
void ITEM_GiveResupplyAmmo(CBasePlayer *pPlayer)
{
...
//	>> CItem_AmmoResupply
// ========================================================================
class CItem_AmmoResupply : public CItem
{
public:
	DECLARE_CLASS(CItem_AmmoResupply, CItem);
...
LINK_ENTITY_TO_CLASS(item_ammoresupply, CItem_AmmoResupply);
...
class CRegenerateZone : public CBaseAnimating
{
public:
	DECLARE_CLASS(CRegenerateZone, CBaseAnimating);
	...
	void	Regenerate(CBasePlayer *pPlayer);
...
LINK_ENTITY_TO_CLASS(item_regenerate, CRegenerateZone);

#define REGENERATE_SOUND				"BaseCombatCharacter.AmmoPickup"
#define REGENERATE_NEXT_USE_TIME		2.0f
...
void CRegenerateZone::Spawn(void)
{
	...
	SetModel("models/items/regenarea.mdl");
	SetMoveType(MOVETYPE_NONE);
	SetSolid(SOLID_VPHYSICS);
	CreateVPhysics();
	...
	SetThink(&CRegenerateZone::ZoneThink);
}
...
{
	PrecacheScriptSound(REGENERATE_SOUND);
	PrecacheModel("models/items/regenarea.mdl");
}

//-----------------------------------------------------------------------------
// Purpose: 
//-----------------------------------------------------------------------------
void CRegenerateZone::ZoneThink(void)
{
	SetNextThink(gpGlobals->curtime + 0.1f);
}
...
// Purpose: 
//-----------------------------------------------------------------------------
void CRegenerateZone::Regenerate(CBasePlayer *pPlayer)
{
	SetNextThink(gpGlobals->curtime + REGENERATE_NEXT_USE_TIME);
	CHL2MP_Player *pHL2MPPlayer = ToHL2MPPlayer(pPlayer);
	pHL2MPPlayer->Regenerate();

	CSingleUserRecipientFilter filter(pHL2MPPlayer);
	EmitSound(filter, pHL2MPPlayer->entindex(), REGENERATE_SOUND);
}
