...
void CSpectatorGUI::UpdateTimer()
{
	...
	int timer = (int)HL2MPRules()->GetMapRemainingTime();
...
void CSpectatorGUI::UpdateScores()
{
	...
	{
		// Ms - Do team scores 
		wchar_t blueTeamScore[6], redTeamScore[6];
		C_Team *teamRed = GetGlobalTeam(TEAM_RED);
		C_Team *teamBlue = GetGlobalTeam(TEAM_BLUE);

		if (teamBlue) {
			swprintf(blueTeamScore, L"%d", teamBlue->Get_Score());
			SetLabelText("CTScoreValue", blueTeamScore);
		}
		if (teamRed) {
			swprintf(redTeamScore, L"%d", teamRed->Get_Score());
			SetLabelText("TERScoreValue", redTeamScore);
		}
	}
	else
	{
		SetLabelText("CTScoreValue", "N/A");
		SetLabelText("TERScoreValue", "N/A");
	}
}
