...
void ClientModeShared::FireGameEvent( IGameEvent *event )
{
	...
	{
		C_BasePlayer *pAttacker = USERID2PLAYER(event->GetInt("attacker"));

		if (pAttacker && pAttacker->IsLocalPlayer())
		{
			if (survivor_player_announcer.GetBool())
			{
				if (survivor_player_announcervoicemode.GetInt() == 0)
				{
					CLocalPlayerFilter filter;
					pAttacker->EmitSound(filter, SOUND_FROM_LOCAL_PLAYER, "AnnouncerACv1.KillEnemy");
				}
				else if (survivor_player_announcervoicemode.GetInt() == 1)
				{
					CLocalPlayerFilter filter;
					pAttacker->EmitSound(filter, SOUND_FROM_LOCAL_PLAYER, "AnnouncerACv2.KillEnemy");
				}
			}

			CLocalPlayerFilter filter;
			pAttacker->EmitSound(filter, SOUND_FROM_LOCAL_PLAYER, "Player.KillEnemy");
		}
	}
	else if (Q_strcmp("player_hurt", eventname) == 0)
	{
		C_BasePlayer *pAttacker = USERID2PLAYER(event->GetInt("attacker"));

		if (pAttacker && pAttacker->IsLocalPlayer())
		{
			if (survivor_hitsound.GetBool())
			{
				CLocalPlayerFilter filter;
				pAttacker->EmitSound(filter, SOUND_FROM_LOCAL_PLAYER, "Player.Hit");
			}
		}
	}
	else if (Q_strcmp("player_spawn", eventname) == 0)
	{
		...
		if (pPlayer && pPlayer->IsLocalPlayer())
		{
			if (survivor_player_announcer.GetBool())
			{
				...
				{
					CLocalPlayerFilter filter;
					pPlayer->EmitSound(filter, SOUND_FROM_LOCAL_PLAYER, "AnnouncerACv1.Respawn");
				}
				else if (survivor_player_announcervoicemode.GetInt() == 1)
				{
					CLocalPlayerFilter filter;
					pPlayer->EmitSound(filter, SOUND_FROM_LOCAL_PLAYER, "AnnouncerACv2.Respawn");
				}
			}
		}
	}
