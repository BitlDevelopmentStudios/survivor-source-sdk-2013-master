...
void IBotSchedule::TaskStart()
{
    ...
    switch ( pTask->task ) {
        ...
        // heal if we have the Healgun.
        case BTASK_HEAL:
        {
            CBaseWeapon* pHealgun = GetHost()->Weapon_OwnsThisType("weapon_healgun");

            if (pHealgun)
            {
                CBaseWeapon* pOldWeapon = GetHost()->GetActiveBaseWeapon();

                if (!pOldWeapon)
                    Fail("No primary weapon found.");
                    break;

                //if we own it, switch to it.
                GetHost()->Weapon_Switch(pHealgun);
                ...
                //switch back to our old weapon.
                if (IsWaitFinished())
                {
                    if (pOldWeapon)
                    {
                        GetHost()->Weapon_Switch(pOldWeapon);
                    }
                    else
                    {
                        Fail("No old weapon found.");
                    }
                }
