...
void DealWithObstacles( CSurvivorBot *pBot, CBaseEntity *pTouchEnt, CUserCmd &cmd  )
{
	if( !pTouchEnt  )
		return;
	...
	/*
	if (FClassnameIs(pTouchEnt, "prop_static"))
	{
	}
	*/
...
void AddRandomPath( CSurvivorBot *pBot, float randomStartAngle = 0 )
{	
...
bool BotOnLadder( CSurvivorBot *pBot )
{
...
bool ArrivedToWaypoint( CSurvivorBot *pBot )
{
...
void CheckStuck( CSurvivorBot *pBot, CUserCmd &cmd )
{
	...
	{			
		...
		if( area )
		{				
			area->IncreaseDanger(pBot->GetTeamNumber(), 5.0f); // let bots know this area is tricky to navigate so they avoid it eventually
		}			
...
void CheckNavMeshAttrib(  CSurvivorBot *pBot, trace_t *ptr, CUserCmd &cmd )
{
...
bool CreatePath( CSurvivorBot *pBot, CBasePlayer *pPlayer, Vector OptionalOrg )
{
	if( !pBot || pBot == NULL )
		return false;
	...
	if( NavAreaBuildPath( startArea, goalArea, &vEnd, costfunc, &closestArea ) )
	{
		...
		while( goalArea->GetParent() )
		{
			...
			// not worrying about ladders for the time being since they don't work. - bitl
			/*
...
bool CreateHidePath( CSurvivorBot *pBot, Vector &HiDeSpot )
{
	DevMsg("Let's hide\n");
...
void SelectSchedule( CSurvivorBot *pBot, bool forcePath = false )
{	
	Vector HideSpot;

	if( CreatePath( pBot, pBot->GetEnemy() ) ) // try to reach enemy
	{
		pBot->m_nBotState = forcePath ? BOT_NAVIG_PATH_ENFORCED : BOT_NAVIG_PATH;
...
bool SafePathAhead( CSurvivorBot *pBot, Vector origin )
{
...
void BotNavigation( CSurvivorBot *pBot, CUserCmd &cmd  )
{
