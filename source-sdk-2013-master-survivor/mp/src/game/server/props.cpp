...
void CBreakableProp::Break( CBaseEntity *pBreaker, const CTakeDamageInfo &info )
{
	...
	if (m_explodeDamage > 0 || m_explodeRadius > 0)
	{
		...
		if (HasInteraction(PROPINTER_PHYSGUN_BREAK_EXPLODE))
		{
			ExplosionCreate(WorldSpaceCenter(), angles, pAttacker, 50.0f, m_explodeRadius,
		...
		else
		{
			...
			ExplosionCreate(WorldSpaceCenter(), angles, pAttacker, 50.0f * flScale, m_explodeRadius * flScale,
