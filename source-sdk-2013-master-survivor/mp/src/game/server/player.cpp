...
ConVar  sv_player_hitsound("sv_player_hitsound", "1");
...
int CBasePlayer::OnTakeDamage_Alive( const CTakeDamageInfo &info )
{
	...
	bool doCrit = false;
	int critrand = random->RandomInt(1,35);
	if (critrand == 35)
	{
		doCrit = true;
	}

	float playerdmg = doCrit ? (info.GetBaseDamage() + CRIT_DMG) : info.GetBaseDamage();
	...
	{
		Vector force = vecDir * -DamageForce(WorldAlignSize(), playerdmg);
		if ( force.z > 250.0f )
		{
			force.z = 250.0f;
		}
	...
	if (info.GetDamageType() & DMG_MINIGUN)
	{
		Vector force = vecDir * -DamageForce(WorldAlignSize(), playerdmg);
		ApplyAbsVelocityImpulse(force * 8.0f);
	}

	if (info.GetDamageType() & DMG_HEALGUN)
	{
		TakeHealth(info.GetBaseDamage(), DMG_GENERIC);
	}
...
//=========================================================
CBaseEntity *CBasePlayer::DropItem(const char *pszItemName, Vector vecPos, QAngle vecAng)
{
	if (!pszItemName)
	{
...
//-----------------------------------------------------------------------------
void CBasePlayer::ForceRespawn(bool PlayAnnouncerSound)
{
	RemoveAllItems( true );
	...
	if (!PlayAnnouncerSound)
	{
		StopSound("AnnouncerACv1.Respawn");
		StopSound("AnnouncerACv2.Respawn");
	}
}
