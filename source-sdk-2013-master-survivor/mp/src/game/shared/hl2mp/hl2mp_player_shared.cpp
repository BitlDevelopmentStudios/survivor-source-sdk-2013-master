...
{
	PrecacheScriptSound( "NPC_Citizen.RunFootstepLeft" );
	PrecacheScriptSound( "NPC_Citizen.RunFootstepRight" );
}
...
void CHL2MP_Player::PlayStepSound( Vector &vecOrigin, surfacedata_t *psurface, float fvol, bool force )
{
	...
	{
		Q_snprintf( szStepSound, sizeof( szStepSound ), "NPC_Citizen.RunFootstepLeft");
	}
	else
	{
		Q_snprintf( szStepSound, sizeof( szStepSound ), "NPC_Citizen.RunFootstepRight");
	}
...
CBaseCombatWeapon *CHL2MP_Player::Weapon_OwnsThisID(int iWeaponID)
{
	return Weapon_OwnsThisType(WeaponIdToClassname(iWeaponID));
}
