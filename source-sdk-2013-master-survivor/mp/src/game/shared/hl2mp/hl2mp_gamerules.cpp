...
void CHL2MPRules::PlayerKilled( CBasePlayer *pVictim, const CTakeDamageInfo &info )
{
	...
	if (pVictim->GetTeamNumber() != TEAM_SPECTATOR && info.GetInflictor() != pVictim && info.GetAttacker() != pVictim)
	{
		if (HL2MPRules()->IsTeamplay())
		{
			if (pVictim->GetTeamNumber() == TEAM_RED)
			{
				pVictim->DropItem("item_dogtag_red", pVictim->WorldSpaceCenter() + RandomVector(-4, 4), RandomAngle(0, 360));
			}
			else if (pVictim->GetTeamNumber() == TEAM_BLUE)
			{
				pVictim->DropItem("item_dogtag_blue", pVictim->WorldSpaceCenter() + RandomVector(-4, 4), RandomAngle(0, 360));
			}
		}
		else
		{
			pVictim->DropItem("item_dogtag", pVictim->WorldSpaceCenter() + RandomVector(-4, 4), RandomAngle(0, 360));
		}
	}

	if (pVictim->GetTeamNumber() == TEAM_SPECTATOR || info.GetInflictor() == pVictim || info.GetAttacker() == pVictim || info.GetInflictor() == pVictim && info.GetAttacker() == pVictim)
	{
		pVictim->StopSound("AnnouncerACv1.KillEnemy");
		pVictim->StopSound("AnnouncerACv2.KillEnemy");
		pVictim->StopSound("Player.KillEnemy");
	}
#endif
}
...
//	float flFragLimit = fraglimit.GetFloat();
	float flScoreLimit = scorelimit.GetFloat();
	float flScoreLimitFFA = scorelimit_ffa.GetFloat();
	...
	if (IsTeamplay() == false)
	{
		if (flScoreLimitFFA)
		{
			...
			for (int i = 1; i <= gpGlobals->maxClients; i++)
			{
				...
				if (pPlayer && pPlayer->FragCount() >= flScoreLimitFFA)
				{
					GoToIntermission();
					return;
				}
	...
	else
	{
		if (flScoreLimit)
		{
			if (IsTeamplay() == true)
			{
				...
				if (pCombine->GetScore() >= flScoreLimit || pRebels->GetScore() >= flScoreLimit)
				{
					GoToIntermission();
					return;
				}
...
{ 
	if ( IsTeamplay() )
		return "SURVIVOR (Team Battle)"; 

	return "SURVIVOR (FFA Battle)"; 
} 
...
void CHL2MPRules::Precache( void )
{
	...
	CBaseEntity::PrecacheScriptSound("Player.Hit");
	CBaseEntity::PrecacheScriptSound("Player.KillEnemy");
	CBaseEntity::PrecacheScriptSound("AnnouncerACv1.KillEnemy");
	CBaseEntity::PrecacheScriptSound("AnnouncerACv2.KillEnemy");
	CBaseEntity::PrecacheScriptSound("AnnouncerACv1.HealthLow");
	CBaseEntity::PrecacheScriptSound("AnnouncerACv2.HealthLow");
	CBaseEntity::PrecacheScriptSound("AnnouncerACv1.Respawn");
	CBaseEntity::PrecacheScriptSound("AnnouncerACv2.Respawn");
	UTIL_PrecacheOther("item_dogtag_red");
	UTIL_PrecacheOther("item_dogtag_blue");
	UTIL_PrecacheOther("item_dogtag");
}
...
CAmmoDef *GetAmmoDef()
{
	...
	if ( !bInitted )
	{
		...
		def.AddAmmoType("AR2",				DMG_BULLET,					TRACER_LINE_AND_WHIZ,	0,			0,			210,			BULLET_IMPULSE(200, 1225),	0 );
		def.AddAmmoType("AR2AltFire",		DMG_DISSOLVE,				TRACER_NONE,			0,			0,			3,			0,							0 );
		def.AddAmmoType("Pistol",			DMG_BULLET,					TRACER_LINE_AND_WHIZ,	0,			0,			120,			BULLET_IMPULSE(200, 1225),	0 );
		def.AddAmmoType("SMG1",				DMG_BULLET,					TRACER_LINE_AND_WHIZ,	0,			0,			225,			BULLET_IMPULSE(200, 1225),	0 );
		def.AddAmmoType("357",				DMG_BULLET,					TRACER_LINE_AND_WHIZ,	0,			0,			120,			BULLET_IMPULSE(800, 5000),	0 );
		def.AddAmmoType("XBowBolt",			DMG_BULLET,					TRACER_LINE,			0,			0,			30,			BULLET_IMPULSE(800, 8000),	0 );
		def.AddAmmoType("Buckshot",			DMG_BULLET | DMG_BUCKSHOT,	TRACER_LINE,			0,			0,			50,			BULLET_IMPULSE(400, 1200),	0 );
		def.AddAmmoType("RPG_Round",		DMG_BURN,					TRACER_NONE,			0,			0,			6,			0,							0 );
		def.AddAmmoType("SMG1_Grenade",		DMG_BURN,					TRACER_NONE,			0,			0,			3,			0,							0 );
		def.AddAmmoType("Grenade",			DMG_BURN,					TRACER_NONE,			0,			0,			15,			0,							0 );
		def.AddAmmoType("slam",				DMG_BURN,					TRACER_NONE,			0,			0,			15,			0,							0 );
		def.AddAmmoType("Gatling",			DMG_BULLET | DMG_MINIGUN,	TRACER_LINE_AND_WHIZ,	0,			0,			500,			BULLET_IMPULSE(200, 1225),	0 );
		def.AddAmmoType("Sniper",			DMG_BULLET | DMG_SNIPER,	TRACER_LINE_AND_WHIZ,	0,			0,			24,			BULLET_IMPULSE(800, 5000),	0 );
		def.AddAmmoType("Recover",			DMG_HEALGUN,				TRACER_LINE,			0,			0,			24,			0,							0);
	}
