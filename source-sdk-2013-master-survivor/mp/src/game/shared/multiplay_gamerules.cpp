...
ConVar scorelimit("mp_scorelimit", "0", FCVAR_NOTIFY | FCVAR_REPLICATED, "The amount of score at which the map ends");

ConVar scorelimit_ffa("mp_scorelimit_ffa", "0", FCVAR_NOTIFY | FCVAR_REPLICATED, "The amount of score at which the map ends");
...
//		float flFragLimit = fraglimit.GetFloat();
		...
		if (flScoreLimit)
		{
			...
			for (int i = 1; i <= gpGlobals->maxClients; i++)
			{
				...
				if (pPlayer && pPlayer->FragCount() >= flScoreLimit)
				{
					GoToIntermission();
					return;
				}
