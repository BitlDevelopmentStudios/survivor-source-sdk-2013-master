...
void CBaseCombatWeapon::AddDefaultAmmo(void)
{
	// If I use clips, set my clips to the default
	if (UsesClipsForAmmo1())
	{
	...
	{
		AddPrimaryAmmoCount(GetDefaultClip1());
		m_iClip1 = WEAPON_NOCLIP;
	}
	...
	{
		AddSecondaryAmmoCount(GetDefaultClip2());
		m_iClip2 = WEAPON_NOCLIP;
	}
}
...
void CBaseCombatWeapon::Precache( void )
{
	...
	if ( ReadWeaponDataFromFileForSlot( filesystem, GetClassname(), &m_hWeaponFileInfo, GetEncryptionKey() ) )
	{
		...
		CBaseEntity::PrecacheModel(GetViewModelRed());
		CBaseEntity::PrecacheModel(GetViewModelBlue());
...
// Purpose: 
//-----------------------------------------------------------------------------
const char *CBaseCombatWeapon::GetViewModelRed(int /*viewmodelindex = 0 -- this is ignored in the base class here*/) const
{
	return GetWpnData().szViewModelRed;
}

//-----------------------------------------------------------------------------
// Purpose: 
//-----------------------------------------------------------------------------
const char *CBaseCombatWeapon::GetViewModelBlue(int /*viewmodelindex = 0 -- this is ignored in the base class here*/) const
{
	return GetWpnData().szViewModelBlue;
}
...
int CBaseCombatWeapon::GetBotWeaponRange(void) const
{
	return GetWpnData().m_iBotWeaponRange;
}
...
{
	//since survivor doesn't need this we're commenting this - bitl
#if !defined( CLIENT_DLL )
...
void CBaseCombatWeapon::Equip( CBaseCombatCharacter *pOwner )
{
	...
	if ( pOwner->IsPlayer() )
	{
		...
		{
			SetModel(GetViewModelBlue());
		}
		else if (pPlayer->GetTeamNumber() == TEAM_RED)
		{
			SetModel(GetViewModelRed());
		}
		else
		{
...
void CBaseCombatWeapon::SetViewModel()
{
	...
	if (pOwner->GetTeamNumber() == TEAM_BLUE)
	{
		vm->SetWeaponModel(GetViewModelBlue(m_nViewModelIndex), this);
	}
	else if (pOwner->GetTeamNumber() == TEAM_RED)
	{
		vm->SetWeaponModel(GetViewModelRed(m_nViewModelIndex), this);
	}
	else
	{
